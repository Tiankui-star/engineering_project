...
-- ==========================================================
-- 3. 教师表和学生表（主键直接关联 sys_user.user_id）
-- ==========================================================
...
-- 学生获奖表（student_id 允许 NULL，外键 SET NULL）
CREATE TABLE `STUDENT_AWARD` (
    ...
    `supervisor_faculty_id` BIGINT COMMENT '指导老师',
...
CREATE TABLE `FUNDING_RECORD` (
    ...
    `funding_type` ENUM('GRANT', 'SPONSORSHIP', 'MATCHING', 'OTHER'),
    `status` ENUM('PENDING', 'RECEIVED', 'SPENT'),
...
-- 项目成员表（支持教师和学生，替代原 RESEARCH_PARTICIPATION）
...
CREATE TABLE `PATENT` (
    ...
    `patent_level` ENUM('NATIONAL', 'PROVINCIAL', 'INTERNATIONAL') COMMENT '专利级别',
...
CREATE TABLE `STATISTICS_VIEW` (
    ...
    `dimension_type` ENUM('GLOBAL', 'DEPARTMENT') NOT NULL,
    `dimension_id` INT,
...
-- ==========================================================
-- 6. 数据大屏实时同步触发器
-- 当业务表数据变化时，自动更新 STATISTICS_VIEW 表
-- ==========================================================
