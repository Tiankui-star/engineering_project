...
public class PatentController extends BaseController
{
    ...
    {
        if (patent.getApplicationDate() != null && patent.getGrantDate() != null
                && patent.getApplicationDate().after(patent.getGrantDate()))
        {
            return error("新增专利失败，申请日期不能晚于授权日期");
        }
        if (patent.getGrantDate() != null && patent.getExpiryDate() != null
                && patent.getGrantDate().after(patent.getExpiryDate()))
        {
            return error("新增专利失败，授权日期不能晚于到期日期");
        }
        if (patent.getApplicationDate() != null && patent.getExpiryDate() != null
                && patent.getApplicationDate().after(patent.getExpiryDate()))
        {
            return error("新增专利失败，申请日期不能晚于到期日期");
        }
    ...
    public AjaxResult edit(@RequestBody Patent patent)
    {
        if (patent.getApplicationDate() != null && patent.getGrantDate() != null
                && patent.getApplicationDate().after(patent.getGrantDate()))
        {
            return error("修改专利失败，申请日期不能晚于授权日期");
        }
        if (patent.getGrantDate() != null && patent.getExpiryDate() != null
                && patent.getGrantDate().after(patent.getExpiryDate()))
        {
            return error("修改专利失败，授权日期不能晚于到期日期");
        }
        if (patent.getApplicationDate() != null && patent.getExpiryDate() != null
                && patent.getApplicationDate().after(patent.getExpiryDate()))
        {
            return error("修改专利失败，申请日期不能晚于到期日期");
        }
