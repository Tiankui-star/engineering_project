...
public class DepartmentController extends BaseController
{
    ...
    /**
     * 查询院系信息管理列表
     * 教职工角色需要加载院系下拉框，学生角色需要展示学生列表中的院系名，因此扩展为 hasAnyPermi
     */
    @PreAuthorize("@ss.hasAnyPermi('Dep:department:list,Faculty:faculty:list,Stu:student:list')")
    @GetMapping("/list")
    ...
    {
        if (!departmentService.checkDepartmentCodeUnique(department))
        {
            return error("新增院系失败，院系代码『" + department.getDepartmentCode() + "』已存在");
        }
        if (!departmentService.checkDepartmentNameUnique(department))
        {
            return error("新增院系失败，院系名称『" + department.getDepartmentName() + "』已存在");
        }
        if (!departmentService.checkHeadFacultyIdUnique(department))
        {
            return error("新增院系失败，该教师已是其他院系的系主任，同一人不能担任多个院系系主任");
        }
    ...
    public AjaxResult edit(@RequestBody Department department)
    {
        if (!departmentService.checkDepartmentCodeUnique(department))
        {
            return error("修改院系失败，院系代码『" + department.getDepartmentCode() + "』已存在");
        }
        if (!departmentService.checkDepartmentNameUnique(department))
        {
            return error("修改院系失败，院系名称『" + department.getDepartmentName() + "』已存在");
        }
        if (!departmentService.checkHeadFacultyIdUnique(department))
        {
            return error("修改院系失败，该教师已是其他院系的系主任，同一人不能担任多个院系系主任");
        }
