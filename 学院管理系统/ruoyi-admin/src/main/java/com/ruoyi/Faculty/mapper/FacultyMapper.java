package com.ruoyi.Faculty.mapper;

import java.util.List;
import com.ruoyi.Faculty.domain.Faculty;

/**
 * 教职工信息及科研统计Mapper接口
 * 
 * @author 软件工程施工队
 * @date 2026-04-07
 */
public interface FacultyMapper 
{
    /**
     * 查询教职工信息及科研统计
     * 
     * @param facultyId 教职工信息及科研统计主键
     * @return 教职工信息及科研统计
     */
    public Faculty selectFacultyByFacultyId(Long facultyId);

    /**
     * 根据工号查询教师
     * 
     * @param facultyNumber 工号
     * @return 教师信息
     */
    public Faculty selectFacultyByFacultyNumber(String facultyNumber);

    /**
     * 查询教职工信息及科研统计列表
     * 
     * @param faculty 教职工信息及科研统计
     * @return 教职工信息及科研统计集合
     */
    public List<Faculty> selectFacultyList(Faculty faculty);

    /**
     * 新增教职工信息及科研统计
     * 
     * @param faculty 教职工信息及科研统计
     * @return 结果
     */
    public int insertFaculty(Faculty faculty);

    /**
     * 修改教职工信息及科研统计
     * 
     * @param faculty 教职工信息及科研统计
     * @return 结果
     */
    public int updateFaculty(Faculty faculty);

    /**
     * 删除教职工信息及科研统计
     * 
     * @param facultyId 教职工信息及科研统计主键
     * @return 结果
     */
    public int deleteFacultyByFacultyId(Long facultyId);

    /**
     * 批量删除教职工信息及科研统计
     * 
     * @param facultyIds 需要删除的数据主键集合
     * @return 结果
     */
    public int deleteFacultyByFacultyIds(Long[] facultyIds);

    /**
     * 同步更新教师工号（由用户管理模块修改用户名时调用）
     *
     * @param facultyId 教师主键（= sys_user.user_id）
     * @param newFacultyNumber 新工号
     * @return 影响行数
     */
    public int updateFacultyNumber(@org.apache.ibatis.annotations.Param("facultyId") Long facultyId,
                                   @org.apache.ibatis.annotations.Param("newFacultyNumber") String newFacultyNumber);
}
